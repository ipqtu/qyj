...
(function () {
    ...
    var tmp = window.location.pathname,
            ...
            URL = "http://localhost:8080/js/public/edit/";//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
    UEDITOR_CONFIG = {
        imagePath:SITE_URL, //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
        ...
        filePath:SITE_URL + "/uploadfiles",  //附件文件夹保存路径
	    catchRemoteImageEnable:true,                                   //是否开启远程图片抓取
        catcherUrl:URL +"/picture/edit_getRemoteImage.html",             //处理远程图片抓取的地址
        localDomain:"baidu.com",                                        //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地
	    imageManagerPath:SITE_URL + "/picture/edit_imageManager.html",       //图片在线浏览的处理地址
        UEDITOR_HOME_URL:URL,                                          //为editor添加一个全局路径
		SITE_URL:SITE_URL,
        //工具栏上的所有的功能按钮和下拉框，可以在new编辑器的实例时选择自己需要的从新定义
		toolbars: [
['Source','ClearDoc','Bold','Italic','Underline','StrikeThrough','BlockQuote','Spechars','|','ForeColor','BackColor','FontFamily','FontSize','CustomStyle','Paragraph','Preview','Help','Print','FullScreen'],['JustifyLeft','JustifyCenter','JustifyRight','JustifyJustify','|','Indent','RowSpacingBottom','RowSpacingTop','LineHeight','|','InsertUnorderedList','InsertOrderedList','Superscript','Subscript','DirectionalityLtr','DirectionalityRtl','|','Horizontal','PageBreak','PastePlain','|','SelectAll','SearchReplace','RemoveFormat','HighlightCode','FormatMatch','Redo','Undo'],['Unlink','Link','Emotion','InsertImage','InsertVideo','Attachment','|','SnapScreen','GMap','Map','Date','Time','|','InsertTable','DeleteTable','MergeRight','MergeDown','SplittoRows','SplittoCols','SplittoCells','MergeCells','InsertCol','InsertRow','DeleteCol','DeleteRow','InsertParagraphBeforeTable'],['']
],
